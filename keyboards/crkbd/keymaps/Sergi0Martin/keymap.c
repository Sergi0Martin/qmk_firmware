...
uint8_t rgbmode;
static void setRGBLayer(int);
bool rgbStatus = true;

enum custom_keycodes {
  KC_Sergio = SAFE_RANGE
};
...
//|--------+--------+--------+--------+--------+--------|                    |--------+--------+--------+--------+--------+--------|
    RGB_MOD, RGB_HUI, RGB_SAI, RGB_VAI, RGB_SPI,   KC_NO,                        BL_UP,   BL_ON,   KC_NO,   KC_NO,   KC_NO,KC_Sergio,
//|--------+--------+--------+--------+--------+--------|                    |--------+--------+--------+--------+--------+--------|
...
void matrix_scan_user(void) {
    if(rgb_matrix_config.mode == RGB_MATRIX_CUSTOM_Base){
        rgbmode = rgb_matrix_config.mode;
    }
}

void setRGBLayer(int n) {
    switch (n) {
        case 0:
            if(!rgbmode) rgbmode = rgb_matrix_config.mode;
            rgb_matrix_mode_noeeprom(RGB_MATRIX_CUSTOM_Base);
            break;
        case 1:
...
layer_state_t layer_state_set_user(layer_state_t state) {
    // uint8_t layer = biton32(get_highest_layer(state));
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case KC_Sergio:
            rgb_matrix_mode(RGB_MATRIX_CUSTOM_Base);
            break;
        case KC_LCTL:
